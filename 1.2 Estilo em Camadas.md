...
> **Localização no mapa arquitetural:** Camadas pertence à **Família 1 — Arquiteturas Monolíticas** apresentada em *1.0.1 — Visão Geral de Arquiteturas de Backend*. Foi introduzido em *1.1 — Estilos Arquiteturais de Backend* (seção 1). Esta seção aprofunda o funcionamento interno, variações (MVC, MVVM, DDD) e exemplos de código do estilo.
...
> **Nota:** MVVM é predominantemente um padrão de **frontend** (WPF, Angular, React). É apresentado aqui como variação do ciclo MVC para fins comparativos, mas não tem papel direto na organização do backend. Os estilos de backend relevantes estão cobertos em *1.1 — Estilos Arquiteturais de Backend*.
