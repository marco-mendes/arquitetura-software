...
1. Monolito Empacotado

Em sistemas simples ou legados, todas as camadas lógicas (apresentação, negócios, persistência e banco) podem ser agrupadas em uma única unidade de implantação. Esse executável pode ser implantado em um único servidor.
...
2. Separação por Responsabilidades Técnicas
...
Exemplo: React no frontend servido via CDN, enquanto backend Node.js com regras de negócio e persistência roda como microserviço.
...
3. Camadas Distribuídas

Outra possibilidade é distribuir camadas individualmente em servidores ou contêineres distintos. Por exemplo, a camada de negócios pode ser isolada como serviço específico, com interface REST ou gRPC; a camada de persistência pode rodar em contêiner separado com regras de acesso.

Exemplo: aplicação .NET Core com Web API (camada de apresentação), serviço gRPC para regras de negócio e microserviço de persistência com Entity Framework, todos implantados em pods distintos no Kubernetes.
...
4. Aplicações de Cliente Rico

Algumas arquiteturas adotam uma abordagem onde a maior parte da lógica de apresentação e até mesmo de negócio está no cliente, como apps desktop ou móveis, que se conectam diretamente a uma API para persistência.

Exemplo: aplicativo Android com regras de negócio implementadas localmente e sincronização com Firebase.
Vantagens: opera offline, resposta imediata ao usuário, menor carga no servidor.
Desvantagens: regras de negócio replicadas em vários clientes, maior esforço de atualização e segurança.
