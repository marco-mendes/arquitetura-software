...
* Apache Camel → Framework de integração que implementa padrões de roteamento e mediação, permitindo orquestrar fluxos de mensagens através de rotas centralizadas. - [Documentação](https://camel.apache.org/)
* NServiceBus → Framework de mensageria distribuída que fornece endpoints e handlers para  - [Documentação](https://particular.net/nservicebus)implementar um Mediator centralizado em arquiteturas orientadas a eventos.
* Spring Integration → Framework de mensageria do ecossistema Spring que fornece canais, handlers e gateways para compor fluxos de mensagens centralizados com padrão Mediator. - [Documentação](https://spring.io/projects/spring-integration)
* AWS Step Functions → Orquestrador serverless que atua como Mediator central, coordenando chamadas a múltiplos serviços com definição explícita de estados, condições e transições. - [Documentação](https://aws.amazon.com/step-functions/)
* Google Workflows → Orquestrador de workflows no Google Cloud que coordena serviços e implementa o  - [Documentação](https://cloud.google.com/workflows/docs)padrão Mediator através de fluxos declarativos com controle de ordem, condições e compensações.
* Azure Logic Apps → Plataforma de orquestração e automação no ecossistema Azure que implementa o padrão Mediator através de fluxos declarativos, orquestrando APIs, filas, funções e conectores. - [Documentação](https://learn.microsoft.com/azure/logic-apps/)
