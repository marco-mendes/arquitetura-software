...
* Permita evolução modular, suportando crescimento e integrações futuras.
  ...
  Vamos explorar a comunicação do hospital com um destes laboratórios.

O Hospital ACME está em uma jornada de modernização, adotando Microsserviços e comunicações REST/JSON. O Laboratório Parceiro é um sistema Monolítico Legado, que exige a comunicação SOAP/XML (padrão TISS) e opera sob alta latência (processamento prolongado). Seu papel é projetar a Interoperabilidade sob estritos requisitos de Segurança (LGPD) e Disponibilidade (99,95%).
